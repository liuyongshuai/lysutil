        ...
        void httpTask::run() {
            ...
            if (expect == "100-continue") {
                ...
                do {
                    ...
                    haveRead += read_size;
                    std::cout << "haveRead=" << haveRead << "\tread_size=" << read_size << "\tcontentLength="
                ...
                *read_size += nread;
                if (nread > 0) {//读到数据
                    ...
                    if (errno == EAGAIN) {//没有数据了
                        ERROR_LOG("ReadThread, read: %d, errno:%d, no data", nread, errno);
                        break;
                    } else if (errno == EINTR) {//可能被内部中断信号打断,经过验证对非阻塞socket并未收到此错误,应该可以省掉该步判断
                        ERROR_LOG("ReadThread, read: %d, errno:%d, interrupt", nread, errno);
                        break;
                    } else {//客户端主动关闭
                        ERROR_LOG("ReadThread, read: %d, errno:%d, peer error", nread, errno);
                        return 1;
                    }
                } else if (nread == 0) {//客户端主动关闭
                    ERROR_LOG("ReadThread, read: %d, errno:%d, peer close", nread, errno);
                    return 1;
                }
            } while (true);
            return 0;
